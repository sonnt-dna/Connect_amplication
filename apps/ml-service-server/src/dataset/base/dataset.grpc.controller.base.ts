/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { GrpcMethod } from "@nestjs/microservices";
import { DatasetService } from "../dataset.service";
import { DatasetCreateInput } from "./DatasetCreateInput";
import { DatasetWhereInput } from "./DatasetWhereInput";
import { DatasetWhereUniqueInput } from "./DatasetWhereUniqueInput";
import { DatasetFindManyArgs } from "./DatasetFindManyArgs";
import { DatasetUpdateInput } from "./DatasetUpdateInput";
import { Dataset } from "./Dataset";
import { TrainingJobFindManyArgs } from "../../trainingJob/base/TrainingJobFindManyArgs";
import { TrainingJob } from "../../trainingJob/base/TrainingJob";
import { TrainingJobWhereUniqueInput } from "../../trainingJob/base/TrainingJobWhereUniqueInput";

export class DatasetGrpcControllerBase {
  constructor(protected readonly service: DatasetService) {}
  @common.Post()
  @swagger.ApiCreatedResponse({ type: Dataset })
  @GrpcMethod("DatasetService", "createDataset")
  async createDataset(
    @common.Body() data: DatasetCreateInput
  ): Promise<Dataset> {
    return await this.service.createDataset({
      data: data,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        name: true,
        source: true,
        size: true,
        format: true,
      },
    });
  }

  @common.Get()
  @swagger.ApiOkResponse({ type: [Dataset] })
  @ApiNestedQuery(DatasetFindManyArgs)
  @GrpcMethod("DatasetService", "datasets")
  async datasets(@common.Req() request: Request): Promise<Dataset[]> {
    const args = plainToClass(DatasetFindManyArgs, request.query);
    return this.service.datasets({
      ...args,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        name: true,
        source: true,
        size: true,
        format: true,
      },
    });
  }

  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: Dataset })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("DatasetService", "dataset")
  async dataset(
    @common.Param() params: DatasetWhereUniqueInput
  ): Promise<Dataset | null> {
    const result = await this.service.dataset({
      where: params,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        name: true,
        source: true,
        size: true,
        format: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: Dataset })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("DatasetService", "updateDataset")
  async updateDataset(
    @common.Param() params: DatasetWhereUniqueInput,
    @common.Body() data: DatasetUpdateInput
  ): Promise<Dataset | null> {
    try {
      return await this.service.updateDataset({
        where: params,
        data: data,
        select: {
          id: true,
          createdAt: true,
          updatedAt: true,
          name: true,
          source: true,
          size: true,
          format: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: Dataset })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @GrpcMethod("DatasetService", "deleteDataset")
  async deleteDataset(
    @common.Param() params: DatasetWhereUniqueInput
  ): Promise<Dataset | null> {
    try {
      return await this.service.deleteDataset({
        where: params,
        select: {
          id: true,
          createdAt: true,
          updatedAt: true,
          name: true,
          source: true,
          size: true,
          format: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Get("/:id/trainingJobs")
  @ApiNestedQuery(TrainingJobFindManyArgs)
  @GrpcMethod("DatasetService", "findManyTrainingJobs")
  async findManyTrainingJobs(
    @common.Req() request: Request,
    @common.Param() params: DatasetWhereUniqueInput
  ): Promise<TrainingJob[]> {
    const query = plainToClass(TrainingJobFindManyArgs, request.query);
    const results = await this.service.findTrainingJobs(params.id, {
      ...query,
      select: {
        id: true,
        createdAt: true,
        updatedAt: true,
        status: true,
        endTime: true,
        startTime: true,

        model: {
          select: {
            id: true,
          },
        },

        dataset: {
          select: {
            id: true,
          },
        },
      },
    });
    if (results === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return results;
  }

  @common.Post("/:id/trainingJobs")
  @GrpcMethod("DatasetService", "connectTrainingJobs")
  async connectTrainingJobs(
    @common.Param() params: DatasetWhereUniqueInput,
    @common.Body() body: TrainingJobWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      trainingJobs: {
        connect: body,
      },
    };
    await this.service.updateDataset({
      where: params,
      data,
      select: { id: true },
    });
  }

  @common.Patch("/:id/trainingJobs")
  @GrpcMethod("DatasetService", "updateTrainingJobs")
  async updateTrainingJobs(
    @common.Param() params: DatasetWhereUniqueInput,
    @common.Body() body: TrainingJobWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      trainingJobs: {
        set: body,
      },
    };
    await this.service.updateDataset({
      where: params,
      data,
      select: { id: true },
    });
  }

  @common.Delete("/:id/trainingJobs")
  @GrpcMethod("DatasetService", "disconnectTrainingJobs")
  async disconnectTrainingJobs(
    @common.Param() params: DatasetWhereUniqueInput,
    @common.Body() body: TrainingJobWhereUniqueInput[]
  ): Promise<void> {
    const data = {
      trainingJobs: {
        disconnect: body,
      },
    };
    await this.service.updateDataset({
      where: params,
      data,
      select: { id: true },
    });
  }
}
